...
class CriticOutput(BaseModel):
    """Critic Agent 的结构化打分输出"""
    geo_score: int = Field(..., description="地理合理性 0-10")
    budget_score: int = Field(..., description="常识与预算匹配度 0-10")
    preference_score: int = Field(..., description="偏好满足度 0-10")
    critique: str = Field(..., description="具体问题描述")
    should_revise: bool = Field(default=False, description="是否需要重做")
    revision_focus: Optional[str] = Field(default=None, description="修改重点")
...
class TripRequest(BaseModel):
    """旅行规划请求"""
    user_id: Optional[str] = Field(default="default_guest", description="用户ID", example="user-123")
...
class TripPlan(BaseModel):
    ...
    exclusive_tips: str = Field(
        default="暂无独家建议",  # 加上默认值，Pydantic 就不会强制要求每次必传了
        description="基于本地私有知识库生成的独家避坑建议..."
    )
...
class TripPlanResponse(BaseModel):
    ...
    critic_scores: Optional[CriticOutput] = None
    consumption_tier: Optional[str] = None
